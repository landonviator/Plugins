...
void LpvtrimmerAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
        {
            ...
            double saturationScaled = pow(10, (*saturation * .05));
            
            if (saturationScaled == 1.0f){
                outBuffer[sample] = (applyGain(input, inGainScaled) * applyPhase(*rawPhase)) * outGainScaled;
            } else {
...
double LpvtrimmerAudioProcessor::applySaturation(double signal, double drive, int force){
